#!/bin/bash
#
#SBATCH --share
#SBATCH --job-name=001_bowtie2
#SBATCH --output=/scratch/gencore/nov_dalma_training/resequencing/hpc-runner/0.05/logs/2016-11-06-hpcrunner_logs/001_bowtie2.log
#SBATCH --partition=serial
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=12
#SBATCH --ntasks-per-node=1
#SBATCH --mem=20GB
#SBATCH --time=01:00:00
#SBATCH --array=1-1

module load gencore gencore_dev gencore_variant_detection/1.0
cd /scratch/gencore/nov_dalma_training/resequencing
hpcrunner.pl execute_array \
	--procs 1 \
	--outdir /scratch/gencore/nov_dalma_training/resequencing/hpc-runner/0.05/scratch \
	--logname 001_bowtie2 \
	--process_table /scratch/gencore/nov_dalma_training/resequencing/hpc-runner/0.05/logs/2016-11-06-hpcrunner_logs/001-task_table.md \
	--metastr '{"total_batches":20,"commands":1,"job_counter":"001","batch":"001","array_end":"1","array_start":"1","jobname":"bowtie2","total_processes":20,"total_jobs":20}' \
	--job_plugins Blog \
	--version 0.05